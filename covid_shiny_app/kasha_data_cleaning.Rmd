...
beds_clean %>% 
  ...
  summarise(Average_Occupancy = mean(percentage)) %>% 
  ggplot() +
    aes(x = date, y = Average_Occupancy, colour = hb_name) +
    geom_line() +
...
```{r}
date <- beds_clean %>% 
  filter(date %in% c("2018-01-01", "2018-01-05")) %>% 
  select(percentage)
